...
func (r *UserRepository) GetByID(ctx context.Context, id uuid.UUID) (*domain.User, error) {
	...
	userDB, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[UserDB])
	if err != nil {
...
func (r *UserRepository) Update(ctx context.Context, user *domain.User) error {
	...
	err := r.db.Pool.QueryRow(ctx, query, user.Username, user.Email, user.PasswordHash, user.ID).Scan(&user.UpdatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
